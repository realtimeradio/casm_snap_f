...
class Sync(Block):
    ...
    def wait_for_sync(self, timeout=20):
        ...
        :return: Sync count after sync has passed
        :rtype int:
        """
        t0 = time.time()
        lastcount = self.count_ext()
        thiscount = self.count_ext()
    ...
    def get_tt_of_sync(self, wait_for_sync=True):
        ...
        if wait_for_sync:
            ...
            sync_number = self.wait_for_sync()
        else:
    ...
    def update_internal_time(self, quiet=False):
        ...
        if count_stop != count_start:
            if not quiet:
                self._error(f"TT loaded after the expected sync arrival (start: {count_start}, stop: {count_stop})!")
            raise RuntimeError(f"TT loaded after the expected sync arrival (start: {count_start}, stop: {count_stop})!")
        # Wait for a sync to pass so the TT is loaded before anything else happens
        self.wait_for_sync()

    def load_timed_sync(self, target_tt):
        """
        Set a timed sync to be emitted when TT = ``target_tt``

        :param target_tt: Target telescope time, in FPGA clocks.
        :type target_tt: int
        """

        assert target_tt < 2**64 - 1
        self.write_int('adc_sync_time_msb', target_tt >> 32)
        self.write_int('adc_sync_time_lsb', target_tt & 0xffffffff)
